...
Mock.mock("/api/blogtype", "get", {
    code: 0, // 错误码
    msg: "", // 错误消息
    "data|10-20": [ //产生10-20个数据对象
        {
            "id|+1": 1, //id从1开始递增
            name: "分类@id", //将id带进去
            "articleCount|0-300": 0, //该分类下文章数量 0-300随机数
            "order|+1": 1,
        },
    ]
})
...
// 模拟分页获取博客请求的响应数据
/**
 * 由于请求带参数，因此匹配路径为/api/blog?参数1=参数值&参数2=参数值
 * 需要拿到路径参数，因此配置虚拟响应数据的时候用函数，其中的option就是地址栏参数
 * /和?都需要用\转义
 * 正则表达式 匹配/api/blog?结尾，其中?出现0次或1次都可以匹配 (\?.+)表示/?后可以出现任意字符一次或多次)
**/
    ...
    return Mock.mock({
        ...
        data: {
            ...
            [`rows|${query.limit || 10}`]: [{
                ...
                // "thumb|1": [Mock.Random.image('200x150', '#000', '#FFF', 'png', 'picture'), null],
                "thumb|1": ["@image(200x150, @color, #fff, @natural)"], 
                createDate: `@date('T')`
            }],
        }
    })
...
// 模拟获取单个博客 匹配路径/api/blog/博客id  (不含有/的任意数据)
Mock.mock(/^\/api\/blog\/[^/]+$/, "get", {
    ...
    data: {
        ...
        "scanNumber|0-1000": 0, //浏览次数 
        "commentNumber|50-200": 0, //评论数 
        description: "@cparagraph(1,10)",//随机产生句子1-10句
        createDate: "@date('T')", //随机产生时间戳
        toc: [ // 博客章节目录      
            //  anchor：锚点# 链接的id
        ...
        <p>这样JS就能够访问指定的响应头了。</p>`, //博客的html内容 
    },
})
...
// 模拟分页获取评论  path： api/comment?page=xxx&limit=xxx
